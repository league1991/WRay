...
void WEnhancedKD::buildEnhancedInterior()
{
	...
	while (stack0.size())
	{
		...
		while (stack1.size())
		{
			...
			interiorMap[srcNode] = (WSKDNode*)currID;			// 把第0层节点地址的对应关系添加到映射
			for (char l1 = 0; l1 < 2; ++l1)
			{
				...
				{	// 第1层节点为内部节点
					...
					interiorMap[l1Node] = (WSKDNode*)(currID | ((1 + l1) << 29));	// 把第1层节点地址的对应关系添加到映射

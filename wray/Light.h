...
public:
	...
	virtual void sampleLight(
		...
		Vector3&iposition,Vector3&iintensity,float&PDF, RandomNumber& rng, MemoryPool& memoryPool)=0;
	virtual void draw()=0;
	//获得属性数组,
...
public:
	...
	{position=iposition;}
	void sampleLight(float u1,float u2,float u3, BSDF&bsdf,Vector3&iposition,Vector3&iintensity,float&PDF, RandomNumber& rng, MemoryPool& memoryPool);
	void draw();
...
public:
	...
	void sampleLight(
		float u1, float u2, float u3, BSDF&bsdf,
		Vector3&iposition, Vector3&iintensity, float&PDF, RandomNumber& rng, MemoryPool& memoryPool);
	void draw(){};
